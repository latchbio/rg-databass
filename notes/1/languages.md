# Language Suggestions

## Implementation

[_The seven programming ur-languages_ by Frederick Ross](https://madhadron.com/programming/seven_ur_languages.html)

### Primary Options

1. [Python](https://www.python.org/)
1. [TypeScript](https://www.typescriptlang.org/) using [NodeJS](https://nodejs.org/) or [Deno](https://deno.com/runtime)
1. [Rust](https://www.rust-lang.org/)
1. [Zig](https://ziglang.org/)
1. [Haskell](https://www.haskell.org/)
1. [Racket](https://racket-lang.org/)
1. [Scala](https://www.scala-lang.org/) or [C#](https://dotnet.microsoft.com/en-us/) or [Java](https://www.java.com/en/)
1. [Swift](https://www.swift.org/)
1. [PureScript](https://www.purescript.org/)
1. [Idris](https://www.idris-lang.org/)
1. [OCaml](https://ocaml.org/)
1. [Agda](https://wiki.portal.chalmers.se/agda/pmwiki.php)
1. [Dafny](https://dafny.org/)
1. [Unison](https://www.unison-lang.org/)
1. [Pharo Smalltalk](https://pharo.org/)

### Secondary Options

1. [Coq](https://coq.inria.fr/)
1. [Go](https://go.dev/)
1. [Vale](https://vale.dev/)

## Modelling

[_The seven specification ur-languages_ by Hillel Wayne (Computer Things #244)](https://buttondown.email/hillelwayne/archive/the-seven-specification-ur-languages/)

- [Alloy](http://alloytools.org/)
- [TLA+](http://lamport.azurewebsites.net/tla/tla.html)
- [Answer Set Programming (Potassco)](https://potassco.org/)
- [P](https://p-org.github.io/P/)
- [PRISM](https://www.prismmodelchecker.org/)
- [Spin](https://spinroot.com/)
- [MCRL2](https://www.mcrl2.org/)
